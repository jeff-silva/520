...
class Posttypes
{
	...
	public function taxonomySearch() {
		...
		foreach(get_post_types() as $posttype) {
			foreach(get_object_taxonomies($posttype, 'objects') as $term) {
				$taxonomy_id = base64_encode($term->name);
				$taxonomies[$taxonomy_id] = array(
					'taxonomy_id' => $taxonomy_id,
					'taxonomy_slug' => $term->name,
					'taxonomy_plural' => $term->labels->name,
					'taxonomy_singular' => $term->labels->singular_name,
					'taxonomy_editable' => false,
					'taxonomy_data' => $term,
				);
			}
		}
